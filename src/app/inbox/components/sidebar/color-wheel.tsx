...
export default function ColorWheel({ h, s, onColorChange }: ColorWheelProps) {
  ...
  const handleInteract = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const saturation = Math.min(100, (Math.sqrt(x * x + y * y) / (size / 2)) * 100);
    onColorChange(Math.round(hue), Math.round(saturation));
  };

  const indicatorPos = {
    x: size / 2 + (s / 100) * (size / 2) * Math.cos(((h - 180) * Math.PI) / 180),
    y: size / 2 + (s / 100) * (size / 2) * Math.sin(((h - 180) * Math.PI) / 180),
  };
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      className="relative size-40 mx-auto cursor-crosshair touch-none"
