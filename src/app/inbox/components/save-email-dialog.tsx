    ...
    <Dialog.Root open={open} onOpenChange={(o) => { if (!o) onConfirm(false); }}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card border border-border rounded-xl p-6 flex flex-col gap-4 shadow-lg w-80">
          <Dialog.Title className="text-sm font-semibold text-foreground">Save email?</Dialog.Title>
          <Dialog.Description className="text-xs text-muted-foreground">Do you wish to save this email before switching?</Dialog.Description>
